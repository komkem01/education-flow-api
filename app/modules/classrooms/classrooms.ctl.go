...
func (c *Controller) handleServiceError(ctx *gin.Context, log *logpkg.Logger, err error, fallback string) {
	...
	case errors.Is(err, ErrClassroomNotFound):
		base.BadRequest(ctx, "classroom-not-found", nil)
	case errors.Is(err, ErrClassroomUnauthorized):
		base.Unauthorized(ctx, "unauthorized", nil)
	case errors.Is(err, ErrClassroomConditionFail):
		base.ValidateFailed(ctx, "condition-fail", nil)
	case errors.Is(err, ErrClassroomDuplicate):
		base.BadRequest(ctx, "classroom-duplicate", nil)
	default:
