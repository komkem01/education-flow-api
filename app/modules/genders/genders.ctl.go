...
func (c *Controller) handleServiceError(ctx *gin.Context, log *logpkg.Logger, err error, fallback string) {
	switch {
	case errors.Is(err, ErrGenderNotFound):
		base.BadRequest(ctx, "gender-not-found", nil)
	case errors.Is(err, ErrGenderUnauthorized):
		base.Unauthorized(ctx, "unauthorized", nil)
	case errors.Is(err, ErrGenderConditionFail):
		base.ValidateFailed(ctx, "condition-fail", nil)
	case errors.Is(err, ErrGenderNameDuplicate):
		base.BadRequest(ctx, "gender-name-duplicate", nil)
	case errors.Is(err, ErrGenderInvalidUpdate):
		base.ValidateFailed(ctx, "condition-fail", nil)
	default:
