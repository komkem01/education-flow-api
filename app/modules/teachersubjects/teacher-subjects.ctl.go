...
func (c *Controller) handleServiceError(ctx *gin.Context, log *logpkg.Logger, err error, fallback string) {
	switch {
	case errors.Is(err, ErrTeacherSubjectNotFound):
		base.BadRequest(ctx, "teacher-subject-not-found", nil)
	case errors.Is(err, ErrTeacherSubjectUnauthorized):
		base.Unauthorized(ctx, "unauthorized", nil)
	case errors.Is(err, ErrTeacherSubjectConditionFail):
		base.ValidateFailed(ctx, "condition-fail", nil)
	case errors.Is(err, ErrTeacherSubjectDuplicate):
		base.BadRequest(ctx, "teacher-subject-duplicate", nil)
	default:
