...
func (c *Controller) handleServiceError(ctx *gin.Context, log *logpkg.Logger, err error, fallback string) {
	...
	case errors.Is(err, ErrSubjectNotFound):
		base.BadRequest(ctx, "subject-not-found", nil)
	case errors.Is(err, ErrSubjectUnauthorized):
		base.Unauthorized(ctx, "unauthorized", nil)
	case errors.Is(err, ErrSubjectConditionFail):
		base.ValidateFailed(ctx, "condition-fail", nil)
	case errors.Is(err, ErrSubjectDuplicate):
		base.BadRequest(ctx, "subject-duplicate", nil)
	default:
