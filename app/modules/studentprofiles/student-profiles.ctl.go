...
func (c *Controller) handleServiceError(ctx *gin.Context, log *logpkg.Logger, err error, fallback string) {
	switch {
	case errors.Is(err, ErrStudentProfileNotFound):
		base.BadRequest(ctx, "student-profile-not-found", nil)
	case errors.Is(err, ErrStudentProfileUnauthorized):
		base.Unauthorized(ctx, "unauthorized", nil)
	case errors.Is(err, ErrStudentProfileConditionFail):
		base.ValidateFailed(ctx, "condition-fail", nil)
	case errors.Is(err, ErrStudentProfileDuplicate):
		base.BadRequest(ctx, "student-profile-duplicate", nil)
	default:
