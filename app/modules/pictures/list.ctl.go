...
func (c *Controller) List(ctx *gin.Context) {
	...
	items, page, err := c.svc.List(ctx.Request.Context(), &req.RequestPaginate, currentUser.Member.SchoolID, ownerMemberID, status)
	if err != nil {
		c.handleServiceError(ctx, log, err, "picture-list-failed")
		return
	}
...
func (c *Controller) PicturesList(ctx *gin.Context) {
	c.List(ctx)
}
