...
func (c *Controller) handleServiceError(ctx *gin.Context, log *logpkg.Logger, err error, fallback string) {
	switch {
	case errors.Is(err, ErrTeacherRequestNotFound):
		base.BadRequest(ctx, "teacher-request-not-found", nil)
	case errors.Is(err, ErrTeacherRequestUnauthorized):
		base.Unauthorized(ctx, "unauthorized", nil)
	case errors.Is(err, ErrTeacherRequestConditionFail):
		base.ValidateFailed(ctx, "condition-fail", nil)
	case errors.Is(err, ErrTeacherRequestDuplicate):
		base.BadRequest(ctx, "teacher-request-duplicate", nil)
	default:
